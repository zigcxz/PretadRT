...
# Output of:
# build_maccs_mordred_from_hmol_standalone_split.py
...
# MACCS + Mordred source-domain RT model.
...
OUTPUT_DIR = os.path.join(
    TUNE_ROOT,
    "fine_tune_maccs_mordred_downstream_result",
    (
        TUNE_MODE
        +
        (
...
def finetune_one(
    ...
    print(
        "MACCS + Mordred downstream fine-tuning:",
        dataset
    )
    ...
    if hasattr(
        ...
        if X_train.shape[
            ...
            raise RuntimeError(
                ...
                f"Tune MACCS+Mordred X dim = {X_train.shape[1]}\n"
    ...
    config = {
        ...
        "molecular_features":
            "MACCS + Mordred are fixed numeric inputs",
...
def main():
    ...
    if not os.path.exists(
        ...
        raise FileNotFoundError(
            "MACCS+Mordred Tune directory not found:\n"
            f"{GEN_DIR}\n\n"
            "Run build_maccs_mordred_from_hmol_standalone_split.py "
            "first."
        )
